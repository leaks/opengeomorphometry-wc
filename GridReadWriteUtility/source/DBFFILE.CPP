...
bool CDbfFile::SaveToFile(CString strFileName, CProgressCtrl* pProgressBar , bool* pbKillThread)
{
	...
	if(!(myFile.Open(strFileName,CFile::modeCreate|CFile::modeWrite )==0))
	{
		...
		for(j=0;j<nNumberOfRecords;j++)
		{
			...
			for(nLoop2=0;nLoop2<nNumOfColumns;nLoop2++)
			{
				...
				if(CType[nLoop2] =='D')
				{
					_itoa(nDec[nLoop2],sNumDec,10);
					strcpy(strParam ,"%.");
					strcat(strParam,sNumDec);
					strcat(strParam,"f");
					//sprintf(pszTemp, "%.3f",((double*)pColumnValue[nLoop2])[j]);

					sprintf(pszTemp, strParam,((double*)pColumnValue[nLoop2])[j]);
				}			
				if(CType[nLoop2] =='F')
				{
					_itoa(nDec[nLoop2],sNumDec,10);
					...
					sprintf(pszTemp, strParam,((float*)pColumnValue[nLoop2])[j]);
...
bool CDbfFile::AddField(CString cszFieldName, char szFieldType, int nSize, int nNumDec)
{
	bool bReturnValue =false;
	if((szFieldType =='I'||szFieldType=='C'||szFieldType == 'S' ||szFieldType =='D'
||
		szFieldType =='F' ||szFieldType =='L' ) && (cszFieldName.GetLength()>=0))
	{
		...
		CType[nNumOfColumns]=szFieldType;
		nFieldSize[nNumOfColumns] = nSize ;
		nNumOfDec[nNumOfColumns] = nNumDec;
		nNumOfColumns++;
		bReturnValue = true;
...
void CDbfFile::InitializeVariables()
{
